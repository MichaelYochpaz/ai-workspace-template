...
This workspace is configured via `ai-workspace.toml` at the repository root. Changes are applied by pre-commit hooks.
...
**Recommended workflow:** After completing tasks, run pre-commit to validate changes and trigger automatic workflows:
```bash
